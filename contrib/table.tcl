...
namespace eval pdf4tcl::pdf4tcl {
	        ...
	        foreach cell $row {
				...
				foreach {1 2 3} [regexp -all -nocase -inline $exp $cell] {
		            switch [string tolower $2] {
    		            "b" { append pdf_cell_format($j,$i) { if {![string match *-bold* [string tolower $pdf(current_font)]]} { $self setFont $font_height $pdf(current_font)-Bold }; } }
        		        "font" {
            		        foreach {param value} [split $3 "= "] {
                		        switch $param {
                    		        "color" { append pdf_cell_format($j,$i) "\$self setFillColor $value; " }
                    		        "size" { append pdf_cell_format($j,$i) "\$self setFont $value; " }
                        		}
		                    }
    		            }
        		    }
	...
	method cell_lengths { str font font_height} {
		...
		foreach row $str {
			...
			foreach cell $row {
				if { ![info exists pdf_row_lengths($i)] } { set pdf_row_lengths($i) 0 }
				if { $mod } { $self setFont $font_height $font; $self setFillColor #000000; set mod 0 }
				# add formatting
				if { $pdf_cell_format($j,$i)!="" } { eval $pdf_cell_format($j,$i); set mod 1 }
	    	    foreach scell [split [string trim $cell] "\n"] {
					set len [expr [$self getStringWidth $scell] + [$self getCharWidth "X"]]
					if { $len > $pdf_row_lengths($i) } { set pdf_row_lengths($i) $len }
				}
				incr i
			}
			incr j
		}
		set endX $startX; foreach {name value} [array get pdf_row_lengths] { set endX [expr $endX + $value] }
	}
...
puts "[$self getTextPosition] - $font_height"
		...
		if { $endX > $XDrawableArea } {
			set i 0; set width 0; # size for bigger columns
			foreach {name value} [array get pdf_row_lengths] {
				if { $value < ($XDrawableArea-$startX)/$pdf_rows } { set width [expr $width + $value]; incr i }
			}
			...
			for { set i 0; set mod 0 } { $i < $pdf_rows } { incr i } {
				...
				if { $pdf_row_lengths($i) > $width } {
					...
					foreach row $str { 
						...
						if { $mod } { $self setFont $font_height $font; $self setFillColor #000000; set mod 0 }
						if { [info exists pdf_cell_format($j,$i)] } { if { $pdf_cell_format($j,$i)!="" } { eval $pdf_cell_format($j,$i); set mod 1 } }
					        ...
					        while {! $done} {
            					...
            					if {($cwidth+$w)>$width || $pos>=$len || $ch=="\n"} {
									if {$pos>=$len} {
                					    ...
                					    if {$lastbp != $start} { set pos $lastbp } else {
			                    	    # Word longer than line, add newline
			    		                    if {$pos > $start} { set txt [string replace $txt $pos $pos "x[string index $txt $pos]"]; incr len }
                	    				}
			...
			unset pdf_row_lengths; $self cell_lengths $str $font $font_height
		}
		# get cell coordinates for backgruond fill.
		set j 0; set ymax [expr [lindex [$self getTextPosition] 1] + $font_height];
puts "[$self getTextPosition] + $font_height"
		set mod 0; set tmp_font_height $font_height
		...
		foreach row $str {
			...
			foreach cell $row {
                if { $mod } { $self setFont $font_height $font; $self setFillColor #000000; set mod 0; set tmp_font_height $font_height }
                # format cell
                if { $pdf_cell_format($j,$i)!="" } { eval $pdf_cell_format($j,$i); set mod 1; set tmp_font_height [$self getFontMetric height] }
